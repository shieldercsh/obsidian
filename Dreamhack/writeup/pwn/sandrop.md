`openat`이랑 `sendfile` 쓰면 된다. 필자는 `sigreturn` 쓰는 게 너무 귀찮고 왠지 안 써도 될 거 같아서 그냥 진행했다. 다른 건 다 문제가 없었는데 `sendfile`의 네 번째 인자(`rcx`)를 조작할 수 없는 게 문제였다. `혹시 어떤 값이 들어있지 않을까?`라고 믿고 해봤는데 1바이트씩 출력되었다. 그래서 offset 조절하면서 일
