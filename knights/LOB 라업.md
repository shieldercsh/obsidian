...
- 익스플로잇 설계
`libc_base`를 알아내고 `system('/bin/sh')`를 실행시키기 위해서는 한 번의 `read`만으로는 부족합니다. 심지어 `main`에서의 `read`함수는 `0x70` 바이트만 읽기 때문에 체이닝하는데 부족한 따라서 스택 피보팅을 이용하겠습니다. 스택 피보팅이란 쓰기 가능한 공간에 가짜 스택 프레임이 있다고 생각하고 `payload`를 작성하는 것입니다. `sfp` 조작으로 `rbp`를 변조할 수 있고, `leave ; ret` 가젯이 있기 때문에 결국 `rsp`를 변조할 수 있어 체이닝을 이어나갈 수 있습니다. `puts` 함수로 `libc_base`를 알아내고, `read`를 다시 한 번 호출하여 `payload`를 작성한 후 `leave ; ret` 가젯으로 `payload`를 실행시켜 쉘을 따줍니다. 이 때 쓰기 가능한 공간은 `PIE`가 꺼져 있으므로 `bss` 영역을 이용합니다.
