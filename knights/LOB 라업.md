...
Chapter 10에서 `libc got overwrite`를 사용하지 않고 풀 수 있는 방법이 두 가지 있습니다. 첫 번째는 `fsb`를 이용하여 `main`의 스택 프레임의 `RET`을 조작하는 것입니다. `one_gadget`을 사용하여도 좋고, `system('/bin/sh')`를 호출하는 체이닝을 짜도 좋습니다. 출력 가능한 `fsb`를 세 번이나 주어주고, `fprintf`에서 길이 `0x64`짜리 `fsb payload`를 실행시켜주므로 체이닝을 짜기 충분할 것으로 생각됩니다.