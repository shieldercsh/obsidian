...
릭 벡터는 `bof`에서 `printf`로 만들어내는 방향으로 설계합니다. `startup` 함수가 종료될 때 `rsi` 값이 `&wish`인 점과 `got` 바이너리에 `PIE`가 꺼져 있는데, `pop rdi ; ret`이 있는 점을 종합하면 `printf("%s", wish)`를 호출할 수 있음을 알 수 있습니다. `wire`의 주소를 알기 때문에 `%s`는 `wire`에 써놓고 참조하도록 합니다. 이렇게 모든 요소를 릭하고 `ROP`를 수행하여 `system('/bin/sh')`를 호출합니다.
