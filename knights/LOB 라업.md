...
```
 `loop` 체크가 `read` 위쪽에 있기 때문에 `0x401260`으로 넘어가면 됩니다. 스택 피보팅을 사용하지 않을 경우 실질적으로 사용할 수 있는 `payload`의 길이가 `0x38`로 약간 짧긴 하지만 `Return to Main`이 더 직관적이고, `main`으로 여러 번 돌아가면 충분히 쉘을 딸 수 있습니다. 여전히 `sfp`를 신경 써야 하는 것은 같지만 이미 짜여져 있는 코드로 돌아가는 것이므로 입문자 입장에서는 조금 더 쉬운 풀이가 될 것 같습니다. `bss``loop` 체크가 `read` 밑에 있어야 의도와 어울릴 것 같습니다.