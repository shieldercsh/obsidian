...
```
`check_id` 함수의 반환값에 따라 실행되는 코드가 결정됩니다. `case 3`의 `printf(id_number);`에서 `0x20`의 길이를 가지는 `payload`를 실행시킬 수 있는 `fsb` 취약점이 발생합니다. `case 0`과 `case 2`에서 호출되는 `check_passwd` 함수의 `fprintf(access_log, passwd);`에서 `0x64`의 길이를 가지는 `payload`를 실행시킬 수 있는 `fsb` 취약점이 발생합니다.
