...
- 익스플로잇 설계
`fsb` 취약점이 존재하면 다양한 방법으로 익스가 가능합니다. 이 문제는 `printf`의 출력을 참고하여`open_emergency_medicine`를 이용하는 방법, `main`의 `RET`을 조작하는 방법이 있고, `printf`의 출력 없이 쉘을 따는 방법이 있습니다. 세 번째 방법은 처음 접할 때 바로 이해하기에는 꽤나 복잡한 과정을 거치기 이 글에서는 소개하지 않겠습니다만, 레이팅이 높은 CTF에서도 `Med - Hard` 난이도의 문제로 종종 출제되는 기법이기 때문에 관심이 있으시다면 익혀두시는 것을 추천합니다. ㅈ 
