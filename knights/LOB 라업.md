...
```
하지만 `libc_start_call_main`은 심볼이 없어서 `offset`을 직접 찾아줘야 합니다. `vmmap` 명령어를 통해 `gdb`상에서 `libc_base`를 찾을 수 있고, 두 값을 빼주면 `offset`을 구할 수 있습니다.(`0x7ffff7db3d90 - 0x7ffff7d8a000 = 0x29d`)
## 피드백
