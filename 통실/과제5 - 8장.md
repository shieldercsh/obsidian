...
추정된 회귀식은 `y = 0.308181(stay) - 0.023005(age) + 0.019661(xray) + 1.001162`이다. `stay, xray`에 대한 검정의 `p-value`는 매우 작으므로 유의수준 5%에서 귀무가설을 기각하여 변수가 유의하다고 할 수 있다. 하지만 `age`에 대한 검정의 `p-value`는 매우 크므로 유의F 검정으로 보아도 `p-value`가 매우 작으므로 유의수준 5%에서 귀무가설을 기각하여 이 회귀 직선은 유의하다고 할 수 있다.