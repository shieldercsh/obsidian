...
추정된 회귀식은 `y = 0.308181(stay) - 0.023005(age) + 0.019661(xray) + 1.001162`이다. 모형의 유의성 검정 결과, 검정통계량은 `20.7`이고 `p-value`는 매우`stay, xray`에 대한 검정의 `p-value`는 매우 작으므로 유의수준 5%에서 귀무가설을 기각하여 변수가 유의하다고 할 수 있다. 하지만 `age`에 대한 검정의 `p-value`는 `0.330098`로 유의수준 5%에서 귀무가설을 기각할 수 없어 변수가 유의하다고 할 수 없다.