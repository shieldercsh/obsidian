...
추정된 회귀식은 `y = 0.308181(ㄴ) + 1.001162`이다. `β_1`에 대한 검정의 `p-value`가 `2e-16`보다 작으므로 유의수준 5%에서 귀무가설을 기각하여 이 회귀 직선은 유의하다고 할 수 있다. F 검정으로 보아도 `p-value`가 매우 작으므로 유의수준 5%에서 귀무가설을 기각하여 이 회귀 직선은 유의하다고 할 수 있다.