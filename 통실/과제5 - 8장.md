...
잔차도를 확인한 결과, 잔차들은 특정한 패턴을 보이거나 등분산성 가정을 위배한 것으로 보 이지는 않는다. 여러 잔차가 범위를 벗어난 큰 값을 갖는 것으로 확인된다. 하지만 정규분위수 그래프를 확인해보면 대부분의 잔차는 직선 주위에 몰려있는 것을 확인할 수 있다. 따라서 단순선형회귀모형의 적용은 타당함을 알 수 있다.
