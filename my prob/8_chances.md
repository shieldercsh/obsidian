...
`password` 초기화 과정에서 알 수 있는 점은 길이가 38인 점, `string.ascii_letters`로 이루어져 있다는 점입니다. 쿼리 실행 결과가 정수형이어야 출력을 볼 수 있으므로 `CAST(... as INT)`를 사용합니다. `INT`는 `BIGINT`로(`9,223,372,036,854,775,807`) 19글자의 숫자로 이루어진 문자열을 정수형으로 바꿀 수 있습니다.
## ex.py
