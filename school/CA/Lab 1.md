...
근데 그러면 ```reg2가 128일 때 success가 1이어야 하는 게 아닌가```라는 의문이 든다. 이는 클럭의 개념을 제대로 알고, `adder`의 작동을 생각하면 이해할 수 있다. `SimpleAdder` 작동을 제외하고 생각하자. `SimpleSystem에서 한 클럭동안 실행되는 것은 무엇일까`를 생각해보면 레지스터 업데이트와 `success` 업데이트이다. 
