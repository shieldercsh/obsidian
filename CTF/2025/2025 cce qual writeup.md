...
`flag`를 얻기 위해 두 가지 작업을 해야 한다. 첫 번째는 `flag`가 저장된 주소를 찾는 것이다. `*($rsp)`가 `code` 영역 주소이므로 `bss` 영역의 시작 주소를 얻을 수 있다. 그로부터 조금 넉넉하게 여유를 두어 약간 앞으로 주소를 잡고 브루트포싱으로 `c` 글자를 탐색한다. 탐색 방법은 `time based sql injection`