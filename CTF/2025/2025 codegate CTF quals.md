...
이걸 쓰고, r12를 0으로 만들어줬다. `FSB`로 `printf` 내부 스택 프레임의 `pop r12`와 ret 부분에 덮어줘서 해결했다. [Merry Christmas](https://shielder.tistory.com/4)문제에서 사용한 기법이다.
...
결과물에서 역연산해준다. `v19`는 만들라는 대로 만들면 된다. 컨볼루션 역과정은, 처음에 가장 바깥 테두리는 0이고, 그 하나 안쪽 테두리는 고정값이라서 확정적으로 익스가 가능하다.
